from lib.nuwave2.lightning_model import NuWave2
...
from lib.nuwave2.utils.stft import STFTMag
import numpy as np
...
class NuWave2Inference:
    def __init__(self, 
        config = 'lib/nuwave2/hparameter.yaml',
        checkpoint = 'lib/nuwave2/nuwave2_02_16_13_epoch=629.ckpt',
        device = 'cuda' if torch.cuda.is_available() else 'cpu'):
        self.hparams = OC.load(config)
        self.model = NuWave2(self.hparams).to(device)
        self.model.eval()
        self.stft = STFTMag()
        self.device = device
    ...
    def infer(self,
        ...
        sound_l = torch.from_numpy(
            sound_l.copy()).float().unsqueeze(0).to(self.device)
        band = band.unsqueeze(0).to(self.device)

        wav_recon, wav_list = self.model.inference(
